...
void GenomeCopyNumber::printRatio(std::string const& outFile, bool ifBedGraphOutPut, bool printNA, bool has_smoothed_profile /*= false*/, bool has_MAF /*= false*/) {
    ...
    if (ifBedGraphOutPut==false) {
        ...
        if (has_smoothed_profile == true) {
            file << "\tSmoothedRatio\tRescaledRatio";
        }
        ...
        if (has_MAF == true) {
            file << "\tSampledBAF\tSampledMAF\tSmoothedMAF";
        }
        if (WESanalysis == true)
        {
            file << "\tGene";
        ...
        file << "\n";
        for ( it=chromosomesInd_.begin() ; it != chromosomesInd_.end(); it++ ) {
            printRatio((*it).first,file,printNA, has_smoothed_profile, has_MAF);
        }
...
void GenomeCopyNumber::printRatio(std::string const& chr, std::string const& outFile, bool printNA, bool has_smoothed_profile, bool has_MAF) {
    cout << "printing ratio for "<<chr<<" into "<<outFile <<"\n";
    std::ofstream file (outFile.c_str());
    printRatio(chr,file,printNA, has_smoothed_profile, has_MAF);
    file.close();
}
...
void GenomeCopyNumber::printRatio(std::string const& chr, std::ofstream & file, bool printNA, bool has_smoothed_profile, bool has_MAF) {
    string::size_type pos = 0;
    string chrNumber = chr;
    if ( ( pos = chrNumber.find("chr", pos)) != string::npos )
        chrNumber.replace( pos, 3, "" );
    ...
    for (int i = 0; i< length; i++) {
        ...
        if (printNA || ratioToPrint!=NA) {//process this this window
            ...
            if (has_smoothed_profile) {
                float smoothedRatioToPrint = chrCopyNumber_[index].getSmoothedRatioAtBin(i);
                file << "\t" << smoothedRatioToPrint;
                float rescaledRatioToPrint = chrCopyNumber_[index].getRescaledRatioAtBin(i);
                file << "\t" << rescaledRatioToPrint;
            }
            ...
            if (has_MAF) {
                file <<"\t"<< chrCopyNumber_[index].getSampledBAFat(i);
                file <<"\t"<< chrCopyNumber_[index].getSampledMAFat(i);
                file << "\t" << chrCopyNumber_[index].getSmoothedMAFat(i);
            }
...
void GenomeCopyNumber::setDataSubsamplingRateInPuritySearch(int _dataSubsamplingRateInPuritySearch)
{
    dataSubsamplingRateInPuritySearch = _dataSubsamplingRateInPuritySearch;
}
...
{
    doRescaleRatio = _doRescaleRatio;
}
...
float GenomeCopyNumber::findMaximumPeak(std::vector<float> total_ratio)
{
    int n = total_ratio.size();
    std::cout << "int n = total_ratio.size(); evals to: " << n << std::endl;
    float max_el = *max_element(total_ratio.begin(), total_ratio.end());
    std::cout << "float max_el = *max_element(total_ratio.begin(), total_ratio.end()); evals to: " << max_el << std::endl;
    std::vector<float> bins(ceil((max_el+0.01)/0.01));
    float x = 0;
    std::generate(bins.begin(), bins.end(), [&]{return x+=0.01;});
    std::cout << "/* for(auto x : bins) */" << std::endl;
    for(auto x : bins)
    {
        std::cout << x << std::endl;  
    }
    std::vector<float> counts = makeHistogramOfCounts(total_ratio, bins);
    std::cout << "Counts: ";
    for (float c : counts) {
        std::cout << c << " ";
    }
...
void GenomeCopyNumber::rescaleRatio()
{
    std::vector<float> total_ratio;
    int count_ratios=0;
    for (unsigned int i = 0; i < chrCopyNumber_.size(); i++) {
        for (unsigned int j = 0; j < chrCopyNumber_[i].getSmoothedRatioSize(); j++) {
            count_ratios++;
            if (i>=chrCopyNumber_[i].getSmoothedRatioSize()) continue;
            float r = chrCopyNumber_[i].getSmoothedRatioAtBin(j);
            if ((r != -1) & (r < 3)) {
                total_ratio.push_back(r);
            }
        }
    }
    std::cout << "rescaleRatio: total_ratio size: " << total_ratio.size() << std::endl;
    std::cout << "rescaleRatio: total_ratio size / all ratios: " << (float)total_ratio.size() / count_ratios << std::endl;
    float peak;
    if (doRescaleRatio){
        peak = findMaximumPeak(total_ratio);
    } else {
        peak = 1;
    }
    std::cout << "Peak: " << peak << std::endl;
    for (unsigned int i = 0; i < chrCopyNumber_.size(); i++) {
        chrCopyNumber_[i].rescaleRatio(peak);
    }
}

void GenomeCopyNumber::preprocessing(SNPinGenome& snpingenome)
{
    for (unsigned int i = 0; i < chrCopyNumber_.size(); i++) {
        chrCopyNumber_[i].createRatioAndBAFFromSNPs(snpingenome);
        chrCopyNumber_[i].transformBAFtoMAF();
        chrCopyNumber_[i].smoothRatioBySegments();
    }
    rescaleRatio();
    for (unsigned int i = 0; i < chrCopyNumber_.size(); i++) {
        chrCopyNumber_[i].simpleExponentialSmoothingMAF();
        chrCopyNumber_[i].filterRatioAndMAF();
    }
}

void GenomeCopyNumber::printRatioAfterPreprocessing(std::string const& outFile)
{
    std::ofstream file (outFile.c_str());
    map<string,int>::iterator it;

    file << "Chromosome\tRatio\tSmoothedRatio\tRescaledRatio\tMedianRatio\tBAF\tMAF\tSmoothedMAF";
    file << "\n";
    for (it=chromosomesInd_.begin() ; it != chromosomesInd_.end(); it++ ) {
        string::size_type pos = 0;
        string chr = (*it).first;
        ...
        int index = findIndex(chrNumber);
        if (index == NA) {return;}
        int length = chrCopyNumber_[index].getSNPLength();
        for (int i = 0; i < length; i++) {
            float ratioToPrint = chrCopyNumber_[index].getRatioPerSNPat(i);
            /*if (isRatioLogged_ && ratioToPrint!= NA) {
                ratioToPrint=pow(2,ratioToPrint);
            }*/
            if (ratioToPrint!=NA) {
                file << chrNumber << "\t" << ratioToPrint;
                float smoothedRatioToPrint = chrCopyNumber_[index].getSmoothedRatioAtBin(i);
                float rescaledRatioToPrint = chrCopyNumber_[index].getRescaledRatioAtBin(i);
                float medianRatioToPrint = chrCopyNumber_[index].getMedianRatioPerSNPat(i);
                file << "\t" << smoothedRatioToPrint << "\t" << rescaledRatioToPrint << "\t" << medianRatioToPrint;
                //int positionToPrint = chrCopyNumber_[index].getSNPPositionAt(i);
                float BAFtoPrint = chrCopyNumber_[index].getBAFPerSNPat(i);
                float MAFtoPrint = chrCopyNumber_[index].getMAFPerSNPat(i);
                float smoothedMAFtoPrint = chrCopyNumber_[index].getSmoothedMAFat(i);
                file << "\t" << BAFtoPrint << "\t" << MAFtoPrint << "\t" << smoothedMAFtoPrint;
                file << "\n";
            }
        }
        cout <<"..writing ratio for " << chr <<"->done!\n";
...
float GenomeCopyNumber::getFilteredRatio(int index, int i)
{
    return chrCopyNumber_[index].getFilteredRatioAt(i);
}

float GenomeCopyNumber::getFilteredMAF(int index, int i)
{
    return chrCopyNumber_[index].getFilteredMAFat(i);
}

void GenomeCopyNumber::fixExpectedRatioAndMAF(GenomeGMM& gmm, const std::vector<float>& ratio, const std::vector<float>& sm_maf, int ploidy, float purity)
{
    gmm.calculateExpectedRatioAndMAF(ploidy, purity);
    gmm.shiftMAF(ratio, sm_maf, ploidy, purity);
}

double GenomeCopyNumber::processGMM(std::string const& outFile, int tau, float alpha, 
bool take_subsample_flag, int take_each_nth_element_for_gmm_fit, bool usePenalty)
{
    map<string,int>::iterator it;
    std::vector<float> genome_ratio_full;
    std::vector<float> genome_ratio_smoothed_full;
    std::vector<float> genome_maf_smoothed_full;
    for (it=chromosomesInd_.begin() ; it != chromosomesInd_.end(); it++ ) {
        string::size_type pos = 0;
        string chrNumber = (*it).first;
        if ( ( pos = chrNumber.find("chr", pos)) != string::npos ) {
        ...
        int index = findIndex(chrNumber);
        if (index != NA) {
            int length_filtered = chrCopyNumber_[index].getFilteredLength();
            for (int i = 0; i < length_filtered; i++) {
                float fratio = getFilteredRatio(index, i);
                float fmaf = getFilteredMAF(index, i);
                genome_ratio_smoothed_full.push_back(fratio);
                genome_maf_smoothed_full.push_back(fmaf);
            }
            ChrCopyNumber cnr = chrCopyNumber_[index];
            int length = cnr.getSNPLength();
            for (int i = 0; i < length; i++) {
                float r = chrCopyNumber_[index].getRatioPerSNPat(i);
                if ((cnr.getRescaledRatioAtBin(i) != -1) && (cnr.getRescaledRatioAtBin(i) < 3) && (cnr.getSmoothedMAFat(i) != -1)) {
                    genome_ratio_full.push_back(r);
                }
            }
        }
    }
    ...
    if (take_subsample_flag){
        std::vector<float> genome_ratio_smoothed_trunc;
        std::vector<float> genome_maf_smoothed_trunc;
        for (int i = 0; i < genome_ratio_smoothed_full.size(); i+=take_each_nth_element_for_gmm_fit)
        {
            genome_ratio_smoothed_trunc.push_back(genome_ratio_smoothed_full.at(i));
            genome_maf_smoothed_trunc.push_back(genome_maf_smoothed_full.at(i));
        }
        genome_ratio_smoothed = genome_ratio_smoothed_trunc;
        genome_maf_smoothed = genome_maf_smoothed_trunc;
    } else {
        genome_ratio_smoothed = genome_ratio_smoothed_full;
        genome_maf_smoothed = genome_maf_smoothed_full;
    }
...
#pragma omp critical
    ...
    if (usePenalty){
        ...
        auto mixtures = sampleGMM.getMixtures();
        std::vector<Eigen::Vector2d> mus;
        for(auto m:mixtures){
            weights.push_back(m.getMixtureComponentWeight());
            covs.push_back(m.getMixtureComponentCov());
            mus.push_back(m.getMixtureComponentMu());
        }
        int N = genome_maf_smoothed.size();
        Eigen::MatrixXd mus2(mus.size(), mus.at(0).size());
        for (int i = 0; i < mus.size(); ++i) {
            mus2.row(i) = mus.at(i);
        }
        penalty = compute_penalty(weights, covs, mus2, N);
    } else {
        penalty = 0;
    }
...
float GenomeCopyNumber::compute_penalty(const std::vector<double>& weights, const std::vector<Eigen::Matrix2d>& covs,
                                        Eigen::MatrixXd mu, int N,
                                        float threshold_empty,
                                        float threshold_noise,
                                        int C
) {
    std::vector<int> empty_clusters;
    std::vector<int> noise_clusters;
    std::vector<int> non_empty_clusters;
    #pragma omp critical
{
    for (int i = 0; i < weights.size(); i++) {
        auto weight = weights[i];
        if (weight <= threshold_empty) {std::cout << "/* weight:\t" << weight << std::endl; empty_clusters.push_back(i);}
        else {non_empty_clusters.push_back(i);}
    }

    for (int i = 0; i < covs.size(); i++) {
        auto cov = covs.at(i)(0, 0);
        if (cov > threshold_noise) {std::cout << "/* cov:\t" << cov << std::endl; noise_clusters.push_back(i);}
    }
    }
    auto inside =
            (mu(empty_clusters, 0).array() <= mu(non_empty_clusters, 0).maxCoeff()) ||
            (mu(empty_clusters, 1).array() <= mu(non_empty_clusters, 1).maxCoeff());
    for (int i = 0; i < empty_clusters.size(); ++i) {
        if (!inside[i])
            empty_clusters.erase(empty_clusters.begin() + i);
    }

    std::vector<int> unwanted_clusters;
    for(int cluster_idx:noise_clusters){
        if(!contains(unwanted_clusters, cluster_idx)){
            unwanted_clusters.push_back(cluster_idx);
        }
    }
    for(int cluster_idx:empty_clusters){
        if(!contains(unwanted_clusters, cluster_idx)){
            unwanted_clusters.push_back(cluster_idx);
        }
    }

    int n_clusters = weights.size();
    int I = n_clusters - unwanted_clusters.size();
    float penalty = C*I*log(N);
    
    std::cout << "PENALTY info:" << std::endl;
    std::cout << "n_clusters:\t" << n_clusters << std::endl;
    std::cout << "threshold_noise:\t" << threshold_noise << std::endl;
    std::cout << "C:\t" << C << std::endl;
    std::cout << "empty_clusters:\t" << empty_clusters.size() << std::endl;
    std::cout << "noise_clusters:\t" << noise_clusters.size() << std::endl;
    std::cout << "unwanted_clusters:\t" << unwanted_clusters.size() << std::endl;
...
// Print the file with the likelihoods
void GenomeCopyNumber::printGMMFinalLikelihoods(std::string const& outFile, const std::vector<std::pair<int, double>>& ploidy_purity_pairs, const std::vector<double> likelihoods, const std::vector<int> number_of_iterations)
{
    std::ofstream file (outFile.c_str());
    file << "Ploidy\tPurity\tFinalLikelihood\tNumberOfIterations";
    file << "\n";
    int n = likelihoods.size();
    if (n != ploidy_purity_pairs.size()) {
        std::cerr << "Error: list of ploidy-purity pairs and list of likelihoods should have the same size!" << std::endl;
    }
    for (int i = 0; i < n; i++) {
        file << ploidy_purity_pairs[i].first << "\t" << ploidy_purity_pairs[i].second << "\t" << likelihoods[i] << "\t" << number_of_iterations[i] << "\n";
    }
    file.close();
}
